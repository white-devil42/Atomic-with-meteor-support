...
public class PrettyPrintTextFormatter implements NbtElementVisitor {
    ...
    public PrettyPrintTextFormatter(String prefix, int indentationLevel, List<String> pathParts) {
        this.prefix = prefix;
    ...
    public void visitList(NbtList element) {
        if (element.isEmpty()) {
            ...
            for (int i = 0; i < element.size(); ++i) {
                ...
                stringBuilder.append((new PrettyPrintTextFormatter(string, this.indentationLevel + 1, this.pathParts)).apply(element.get(i)));
                if (i != element.size() - 1) {
